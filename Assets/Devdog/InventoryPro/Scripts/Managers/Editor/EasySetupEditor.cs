...
using DevdogEditors = Devdog.General.Editors;
            ...
            if(managers != null)
            {
                ...
                if (inventoryManager != null)
                {
                    if (GUILayout.Button("Generate all databases"))
                    {
                        ...
                        {
                            inventoryManager.sceneLangDatabase = DevdogEditors.ScriptableObjectUtility.CreateAsset<LangDatabase>(path, "LangDatabase.asset", false);
                            generalSettingsManager.settings = DevdogEditors.ScriptableObjectUtility.CreateAsset<GeneralSettings>(path, "GeneralSettings.asset", false);
                            ...
                            var enUs = DevdogEditors.ScriptableObjectUtility.CreateAsset<Devdog.General.Localization.LocalizationDatabase>(path, "enUS.asset", false);
                            enUs.lang = "en-US";
                            dbField.SetValue(localizationManager, new [] { enUs });

                            inventorySettingsManager.settings = DevdogEditors.ScriptableObjectUtility.CreateAsset<InventorySettingsDatabase>(path, "InventorySettings.asset", false);
                            itemManager.sceneItemDatabase = DevdogEditors.ScriptableObjectUtility.CreateAsset<ItemDatabase>(path, "ItemDatabase.asset", false);

                            inventorySettingsManager.settings.collectionSorter = DevdogEditors.ScriptableObjectUtility.CreateAsset<BasicCollectionSorter>(path, "BasicCollectionSorter.asset", false);
                            inventorySettingsManager.settings.itemDropHandler = DevdogEditors.ScriptableObjectUtility.CreateAsset<ItemDropHandler>(path, "ItemDropHandler.asset", false);
