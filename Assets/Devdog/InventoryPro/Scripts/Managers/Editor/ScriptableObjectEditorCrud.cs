...
namespace Devdog.InventoryPro.Editors
{
    public abstract class ScriptableObjectEditorCrud<T> : InventoryEditorCrudBase<T> where T : ScriptableObject
    {
        ...
        {
            var item = DevdogEditors.ScriptableObjectUtility.CreateAsset<T>(InventoryScriptableObjectUtility.GetSaveFolderForType(typeof(T)), DateTime.Now.ToFileTimeUtc() + ".asset");
            GiveItemNewID(item);
            AddItem(item, true);
        }
