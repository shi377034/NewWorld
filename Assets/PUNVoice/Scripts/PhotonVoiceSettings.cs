...
//    public string AppVersion = "1.0";
    ...
    public static PhotonVoiceSettings Instance { 
        ...
        {
            instance = value;
            var objs = Resources.FindObjectsOfTypeAll<PhotonVoiceSettings>();
            if (objs.Length > 1) 
            {
                Debug.LogError("PUNVoice: Multiple instances of PhotonVoiceSettings:");
                foreach (var s in objs)
                {
                    var p = s.ToString();
                    for (var t = s.gameObject.transform.parent; t != null; t = t.parent)
                    {
                        p = t.name + "/" + p;
                    }
                    Debug.LogError("\t" + p);
