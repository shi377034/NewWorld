...
namespace Sirenix.Serialization
{
    ...
    public class ReflectionFormatter<T> : BaseFormatter<T>
    {
        ...
        protected override void DeserializeImplementation(ref T value, IDataReader reader)
        {
            ...
            var members = FormatterUtilities.GetSerializableMembersMap(typeof(T), reader.Context.Config.SerializationPolicy);
            ...
            while ((entryType = reader.PeekEntry(out name)) != EntryType.EndOfNode && entryType != EntryType.EndOfArray && entryType != EntryType.EndOfStream)
            {
                ...
                {
                    reader.Context.Config.DebugContext.LogError("Entry of type \"" + entryType + "\" in node \"" + reader.CurrentNodeName + "\" is missing a name, at position " + reader.Stream.Position + ".");
                    reader.SkipEntry();
                    continue;
                }
                ...
                {
                    reader.Context.Config.DebugContext.LogWarning("Lost serialization data for entry \"" + name + "\" of type \"" + entryType + "\"in node \"" + reader.CurrentNodeName + "\", at stream position " + reader.Stream.Position + ".");
                    reader.SkipEntry();
                    continue;
                }
        ...
        {
            var members = FormatterUtilities.GetSerializableMembers(typeof(T), writer.Context.Config.SerializationPolicy);
