        ...
        protected override void DeserializeImplementation(ref T value, IDataReader reader)
        {
            ...
            if (invocationList != null)
            {
                ...
                if (combinedDelegate != null)
                {
                    ...
                    {
                        reader.Context.Config.DebugContext.LogError("Could not cast recombined delegate of type " + combinedDelegate.GetType().GetNiceFullName() + " to expected delegate type " + typeof(T).GetNiceFullName() + ".");
                    }
                }
            ...
            {
                reader.Context.Config.DebugContext.LogError("Missing declaring type of delegate on deserialize.");
                return;
            }
            ...
            if (methodInfo == null)
            {
                if (useSignature)
                {
                    reader.Context.Config.DebugContext.LogError("Could not find method with signature " + name + "(" + string.Join(", ", signature.Select(p => p.GetNiceFullName()).ToArray()) + ") on type '" + declaringType.FullName + (wasAmbiguous ? "; resolution was ambiguous between multiple methods" : string.Empty) + ".");
                }
                else
                {
                    reader.Context.Config.DebugContext.LogError("Could not find method with name " + name + " on type '" + declaringType.GetNiceFullName() + (wasAmbiguous ? "; resolution was ambiguous between multiple methods" : string.Empty) + ".");
                }
            ...
            if (methodInfo.IsGenericMethodDefinition)
            {
                ...
                {
                    reader.Context.Config.DebugContext.LogError("Method '" + declaringType.GetNiceFullName() + "." + methodInfo.GetNiceName() + "' of delegate to deserialize is a generic method definition, but no generic arguments were in the serialization data.");
                    return;
                }
                ...
                {
                    reader.Context.Config.DebugContext.LogError("Method '" + declaringType.GetNiceFullName() + "." + methodInfo.GetNiceName() + "' of delegate to deserialize is a generic method definition, but there is the wrong number of generic arguments in the serialization data.");
                    return;
                }
                ...
                for (int i = 0; i < genericArguments.Length; i++)
                {
                    ...
                    {
                        reader.Context.Config.DebugContext.LogError("Method '" + declaringType.GetNiceFullName() + "." + methodInfo.GetNiceName() + "' of delegate to deserialize is a generic method definition, but one of the serialized generic argument types failed to bind on deserialization.");
                        return;
                    }
                }
            ...
            else
            {
                ...
                if (typeof(UnityEngine.Object).IsAssignableFrom(targetType))
                {
                    ...
                    {
                        reader.Context.Config.DebugContext.LogError("Method '" + declaringType.GetNiceFullName() + "." + methodInfo.GetNiceName() + "' of delegate to deserialize is an instance method, but Unity object target of type '" + targetType.GetNiceFullName() + "' was null on deserialization. Did something destroy it?");
                        return;
                    }
                }
                else
                {
                    ...
                    {
                        reader.Context.Config.DebugContext.LogError("Method '" + declaringType.GetNiceFullName() + "." + methodInfo.GetNiceName() + "' of delegate to deserialize is an instance method, but no valid instance target of type '" + targetType.GetNiceFullName() + "' was in the serialization data. Has something been renamed since serialization?");
                        return;
                    }
                }
            ...
            if (value == null)
            {
                reader.Context.Config.DebugContext.LogError("Failed to create delegate of type " + delegateType.GetNiceFullName() + " from method '" + declaringType.GetNiceFullName() + "." + methodInfo.GetNiceName() + "'.");
                return;
            }
