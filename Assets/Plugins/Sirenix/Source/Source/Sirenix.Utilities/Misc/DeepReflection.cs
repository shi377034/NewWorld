        ...
        private static List<PathStep> GetMemberPath(Type rootType, ref Type resultType, string path, out bool rootIsStatic, bool isSet)
        {
            ...
            for (int i = 0; i < steps.Length; i++)
            {
                ...
                if (step.StartsWith("[", StringComparison.InvariantCulture) && step.EndsWith("]", StringComparison.InvariantCulture))
                {
                    ...
                    {
                        throw new ArgumentException("Cannot get elements by index from the type '" + currentType.GetNiceName() + "'.");
                    }
                ...
                {
                    throw new ArgumentException("The method '" + member.GetNiceName() + "' has no return type and cannot be part of a deep reflection path.");
                }
            ...
            {
                throw new ArgumentException("Last member of path '" + result[result.Count - 1].Member.GetNiceName() + "' contains type '" + currentType.GetNiceName() + "', which is not assignable to expected type '" + resultType.GetNiceName() + "'.");
            }
