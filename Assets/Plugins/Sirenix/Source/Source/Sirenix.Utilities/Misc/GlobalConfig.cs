...
namespace Sirenix.Utilities
{
    ...
    public abstract class GlobalConfig<T> : ScriptableObject where T : GlobalConfig<T>, new()
    {
        ...
        public static T Instance
        {
            get
            {
                if (GlobalConfig<T>.instance == null)
                {
                    ...
                    {
                        Debug.LogWarning(ConfigAttribute.AssetPath + typeof(T).GetNiceName() + ".asset" + " was prevented from beeing generated because the PREVENT_SIRENIX_FILE_GENERATION key was defined in Unity's EditorPrefs.");
        ...
        {
#if UNITY_EDITOR
            var configAttribute = this.GetType().GetAttribute<EditorGlobalConfigAttribute>();
            if (configAttribute == null)
            {
                Debug.Log("No EditorGlobalConfigAttribute defined on " + this.GetType().Name + ".");
            }
            else
            {
                configAttribute.OpenWindow(this);
            }
#else
