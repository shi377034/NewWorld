...
namespace Sirenix.Utilities
{
    ...
    public static class TypeExtensions
    {
        ...
        public static Func<T, T, bool> GetEqualityComparerDelegate<T>()
        {
            ...
            MethodInfo equalityMethod = typeof(T).GetOperatorMethod(Operator.Equality); // TODO: Account for equality methods for other types
        ...
        public static IEnumerable<MemberInfo> GetAllMembers(this Type type, string name, BindingFlags flags = BindingFlags.Default)
        {
            ...
            do
            {
                foreach (var member in currentType.GetMember(name, flags))
                {
                    yield return member;
                }
        ...
        /// <summary>
        /// Gets all members of a specific type from a type, including members from all base types.
        /// </summary>
        ...
        {
            return GetBaseClasses(type, includeSelf).Concat(type.GetInterfaces());
        }
        ...
        private static bool TypeFulfillsGenericParameter(this Type genericParameterDefinition, Type parameterType, Dictionary<Type, Type> resolvedMap)
        {
            ...
            for (int i = 0; i < constraints.Length; i++)
            {
                ...
                {
                    if (!constraint.TypeFulfillsGenericParameter(parameterType, resolvedMap))
                    {
                        return false;
                    }
                }
                else if (constraint.IsClass || constraint.IsInterface)
                {
                    if (constraint.IsGenericType)
                    {
                        Type constraintDefinition = constraint.GetGenericTypeDefinition();
                        ...
                        for (int j = 0; j < constraintParams.Length; j++)
                        {
                            ...
                            {
                                if (!TypeFulfillsGenericParameter(c, p, resolvedMap))
                                {
                                    return false;
                                }
                            }
        ...
        public static bool GenericArgumentsContainsTypes(this Type type, params Type[] types)
        {
            ...
            while (argsToCheck.Count > 0)
            {
                ...
                // If argument is a generic type, we
