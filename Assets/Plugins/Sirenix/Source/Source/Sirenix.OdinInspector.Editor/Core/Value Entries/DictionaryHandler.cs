...
namespace Sirenix.OdinInspector.Editor
{
    ...
    public class DictionaryHandler<TDictionary, TKey, TValue> : IDictionaryHandler<TKey> where TDictionary : IDictionary<TKey, TValue>
    {
        ...
        private PropertyValueEntry<TDictionary> valueEntry;
        ...
        public DictionaryHandler(PropertyValueEntry<TDictionary> valueEntry)
        {
                ...
                switch (change.ChangeType)
                {
                    case DictionaryChangeType.SetValue:
                        for (int i = 0; i < this.valueEntry.ValueCount; i++)
                        {
                            ...
                            if (this.valueEntry.SerializationBackend == SerializationBackend.Odin && this.valueEntry.Property.Tree.HasPrefabs)
                            {
                                this.valueEntry.Property.Tree.RegisterPrefabDictionaryAddKeyModification(this.valueEntry.Property, i, change.Key);

                                var child = this.valueEntry.Property.Children[InspectorSerializationUtility.GetDictionaryKeyString(change.Key)];
            ...
            if (this.valueEntry.Property.Tree.UpdateID != this.lastUpdateID)
            {
                ...
                for (int i = 0; i < this.keys.Length; i++)
                {
                    ...
                    if (keyList.Count > 1)
                    {
                        ...
                        {
                            var b = keyList[j];
                            if (Comparer<TKey>.Default.Compare(a, b) > 0)
                            {
                                keyList.Sort();
                                break;
                            }
