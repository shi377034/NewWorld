...
namespace Sirenix.OdinInspector.Editor
{
    ...
    {
        private Dictionary<object, int> objectReferenceCounts = new Dictionary<object, int>();
        private Dictionary<object, string> objectReferences = new Dictionary<object, string>();
        ...
        private void UpdatePrefabsAndModifications()
        {
            ...
            if (typeof(UnityEngine.Object).IsAssignableFrom(this.TargetType))
            {
                ...
                for (int i = 0; i < this.WeakTargets.Count; i++)
                {
                    ...
                    var prefab = (T)(object)PrefabUtility.GetPrefabParent(target);

                    if (prefabType == PrefabType.PrefabInstance && prefab != null)
                    {
                        prefabs[i] = prefab;
            ...
            if (result != null && this.allowAutoRegisterPrefabModifications && registerModification && this.allTargetsHaveSamePrefab && Event.current.type == EventType.Repaint)
            {
                switch (result.Value)
                {
                    case PrefabModificationType.Value:
                        ...
                        {
                            //Debug.Log(this.updateID + " (id): Registering non-inspector-triggered prefab value change for property " + property.Path);
        ...
        private static bool PropertyCanHaveModifications(InspectorProperty property)
        {
            ...
            if (property.ValueEntry != null && (property.ValueEntry.ValueCategory == PropertyValueCategory.DictionaryElement || (property.Name == "Key" && property.Parent != null && property.Parent.ValueEntry != null && property.Parent.ValueEntry.ValueCategory == PropertyValueCategory.DictionaryElement)))
            {
                return false;
            }
        ...
        {
            if (Application.isPlaying || !this.HasPrefabs || !this.allTargetsHaveSamePrefab || !PropertyCanHaveModifications(property))
            {
            ...
            if (prefabProperty.ValueEntry.ValueState != property.ValueEntry.ValueState)
            {
                // Different value states means there some sort of difference
                return PrefabModificationType.Value;
            }
            ...
            {
                // If it's a unity object, always check directly against the references in the prefab
                return PrefabModificationType.Value;
            }
            ...
            if (prefabProperty.Children != null && property.Children != null && prefabProperty.Children.IsCollection && prefabProperty.Children.Count != property.Children.Count)
            {
                ...
                if (prefabProperty.ValueEntry.ValueIsDictionary)
                {
                    return PrefabModificationType.Dictionary;
                }
                else
        ...
        public override void RegisterPrefabListLengthModification(InspectorProperty property, int targetIndex, int newLength)
        {
            if (!TargetSupportsPrefabSerialization)
            {
                Debug.LogError("Target of type " + typeof(T) + " does not support prefab serialization!");
                return;
            }
        ...
        public override void RegisterPrefabValueModification(InspectorProperty property, int targetIndex, bool forceImmediate = false)
        {
            ...
            // Initialize modification with property values and reference paths
            {
                ...
                if (!object.ReferenceEquals(value, null) && (property.ValueEntry.ValueState == PropertyValueState.Reference || property.Info.PropertyType == PropertyType.ReferenceType))
                {
                    string refPath;
            ...
            {
                this.UpdatePrefabsAndModifications();
                this.RemovePrefabModificationsForInvalidPaths("", listLengthMods, valueMods, dictionaryMods);
        ...
        public override void RegisterPrefabDictionaryDeltaModification(InspectorProperty property, int targetIndex)
        {
            ...
            for (int i = 0; i < prefabProperty.Children.Count; i++)
            {
                ...
                if (propChild == null)
                {
                    ...
                    object key = InspectorSerializationUtility.GetDictionaryKeyValue(prefabChild.Name);
            ...
            for (int i = 0; i < property.Children.Count; i++)
            {
                ...
                {
                    // This is a removed key
                    object key = InspectorSerializationUtility.GetDictionaryKeyValue(propChild.Name);
        ...
        public override void RemovePrefabDictionaryModification(InspectorProperty property, int targetIndex, object key)
        {
            ...
            if (!TargetSupportsPrefabSerialization)
            {
                Debug.LogError("Target of type " + typeof(T) + " does not support prefab serialization; implement the ISupportsPrefabSerialization interface!");
                return;
            }
            ...
            if (dictionaryMods.TryGetValue(removePath, out mod))
            {
                ...
                if (mod.DictionaryKeysAdded != null)
                {
                    ...
                    if (changed)
                    {
                        ...
                        string checkPath = removePath + "." + InspectorSerializationUtility.GetDictionaryKeyString(key);
            ...
            else if (property.ValueEntry.SerializationBackend == SerializationBackend.Odin)
            {
                ...
                else if (modificationType == PrefabModificationType.Dictionary && dictionaryMods.TryGetValue(removePath, out mod))
                {
                    ...
                    if (mod.DictionaryKeysAdded != null)
                    {
                        ...
                        for (int i = 0; i < mod.DictionaryKeysAdded.Length; i++)
                        {
                            string keyStr = InspectorSerializationUtility.GetDictionaryKeyString(mod.DictionaryKeysAdded[i]);
        ...
        private void RemovePrefabModificationsForInvalidPaths(string startPath, Dictionary<string, PrefabModification> listLengthMods, Dictionary<string, PrefabModification> valueMods, Dictionary<string, PrefabModification> dictionaryMods)
        {
            HashSet<string> toRemove = new HashSet<string>();
            ...
            foreach (string path in listLengthMods.Keys.Append(valueMods.Keys).Append(dictionaryMods.Keys))
            {
                ...
                if (this.GetPropertyAtDeepReflectionPath(path) == null)
                {
                    toRemove.Add(path);
                }
            }
        ...
        public override SerializedProperty GetUnityPropertyForPath(string path, out FieldInfo backingField)
        {
            ...
            if (result == null && prop != null && (prop.Info.PropertyType == PropertyType.ReferenceType || prop.Info.PropertyType == PropertyType.ValueType))
            {
                ...
                {
                    // 2017.1 broke creation of emitted scriptable objects, so this feature is disabled in that version of Unity
                    result = null;
                }
                else
                {
                    ...
                    // TargetObject is sometimes destroyed, often when the profiler is toggled. Not sure why, but we need to handle the case.
        ...
        internal override void ForceRegisterObjectReference(object reference, InspectorProperty property)
        {
            ...
            {
                // We do not allow references to boxed value types
                return;
            }
