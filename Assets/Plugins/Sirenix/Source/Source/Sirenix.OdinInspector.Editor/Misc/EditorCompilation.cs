    ...
    public sealed class {EditorNamePrefix}_Editor : {BaseEditorType}
    {
        ...
        private static void LogEditorRecompilationMessage(TypeDrawerPair[] editors)
        {
            ...
            else
            {
                ...
                HashSet<string> newEditors = editors.Select(x => InspectorTypeDrawingConfig.TypeBinder.BindToType(x.DrawnTypeName).GetNiceFullName()).ToHashSet();
        ...
        public static bool CompileEditors(TypeDrawerPair[] editors)
        {
            ...
            using (AssemblyGenerator generator = new AssemblyGenerator())
            {
                ...
                List<Type> reservedTypeNames = new List<Type>();

                // Create extern aliases for all assembies to prevent type name conflicts during compilation
                {
                    foreach (var typeDrawerPair in editors)
                    {
                        ...
                        if (ReservedKeywords.Contains(drawnType.FullName))
                        {
                            reservedTypeNames.Add(drawnType);
                        }
                ...
                // drawn types or base editors are no longer there, and trigger a recompile.
                {
                    ...
                    foreach (TypeDrawerPair typeDrawerPair in editors)
                    {
                        ...
                        if (drawnType == null || editorType == null || ReservedKeywords.Contains(drawnType.FullName))
                        {
                            continue;
                        }
                ...
                if (editors.Length > 0)
                {
                    ...
                    foreach (TypeDrawerPair typeDrawerPair in editors)
                    {
                        ...
                        if (drawnType != null && editorType != null && !drawnType.IsGenericTypeDefinition && InspectorTypeDrawingConfig.UnityInspectorEditorIsValidBase(editorType, drawnType) && !ReservedKeywords.Contains(drawnType.FullName))
                        {
                            generator.AddReferencedAssembly(drawnType.Assembly);
                            generator.AddReferencedAssembly(editorType.Assembly);
                ...
                {
                    Debug.LogWarning("Warning: Attempted to generate editors for types with reserved names. The types have been disabled in future editor compiles. Odin is unable to generate editors for any types with reserved names. The following " + reservedTypeNames.Count + " warnings detail the names in question:");

                    foreach (var type in reservedTypeNames)
                    {
                        InspectorConfig.Instance.DrawingConfig.SetEditorType(type, null);
                        Debug.LogWarning("A type with the reserved name '" + type.FullName + "' was declared in the assembly '" + type.Assembly.FullName + "'.");
                    }
