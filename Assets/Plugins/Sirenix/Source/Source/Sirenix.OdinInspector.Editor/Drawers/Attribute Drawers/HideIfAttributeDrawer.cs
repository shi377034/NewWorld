...
namespace Sirenix.OdinInspector.Editor.Drawers
{
    ...
    public sealed class HideIfAttributeDrawer : OdinAttributeDrawer<HideIfAttribute>
    {
        ...
        {
            var config = property.Context.Get(this, "ShowIfConfig", (HideIfConfig)null);

            if (config.Value == null)
            {
                config.Value = new HideIfConfig();
            ...
            else
            {
                ...
                if (config.Value.InstanceMemberGetter != null)
                {
                    ...
                    {
                        if (config.Value.InstanceMemberGetter(property.ParentValues[i]))
                        {
                            drawProperty = false;
                            break;
                        }
                ...
                else if (config.Value.StaticMemberGetter != null)
                {
                    for (int i = 0; i < property.ParentValues.Count; i++)
                    {
                        if (config.Value.StaticMemberGetter())
                        {
                            drawProperty = false;
                            break;
                        }
