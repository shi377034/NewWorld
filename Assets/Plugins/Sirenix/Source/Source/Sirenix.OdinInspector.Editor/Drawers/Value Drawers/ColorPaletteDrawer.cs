    ...
    internal sealed class ColorPaletteDrawer : OdinValueDrawer<ColorPalette>
    {
        ...
        protected override void DrawPropertyLayout(IPropertyValueEntry<ColorPalette> entry, GUIContent label)
        {
            ...
            SirenixEditorGUI.BeginBox();
            {
                ...
                if (SirenixEditorGUI.BeginFadeGroup(entry.SmartValue, entry, isEditing.Value, GeneralDrawerConfig.Instance.GUIFoldoutAnimationDuration))
                {
                    this.CallNextDrawer(entry.Property, null);
                }
                SirenixEditorGUI.EndFadeGroup();

                if (SirenixEditorGUI.BeginFadeGroup(entry.SmartValue, entry.SmartValue, isEditing.Value == false, GeneralDrawerConfig.Instance.GUIFoldoutAnimationDuration))
                {
