    ...
    public class LocalVoiceFramed<T> : LocalVoice
    {
        ...
        : base(voiceClient, encoder, id, voiceInfo, channelId)
        {
            ...
            if (x[0] is byte)
                ...
                throw new Exception(LogPrefix + ": input data type is not supported: " + x[0].GetType());

        }

        int sourceFramePos = 0;

        private IEnumerable<T[]> frameToSourceFrameSize(T[] buf)
        {
            var s = FrameSize;
            // quick return in trivial case
            if (s == buf.Length && sourceFramePos == 0)
            {
                yield return buf;
            }
            else
            {
                ...
                while (bufPos + s - sourceFramePos <= buf.Length)
                {
                    var l = s - sourceFramePos;
                    Buffer.BlockCopy(buf, bufPos * sizeofT, frame, sourceFramePos * sizeofT, l * sizeofT);
        ...
        public void PushDataAsync(T[] buf)
        {
            ...
            lock (pushDataQueue)
            {
                // Caller should check this asap in general case if packet production is expencive.
        ...
        public void PushData(T[] buf)
        {
            if (this.voiceClient.frontend.IsChannelJoined(this.channelId) && this.Transmit)
            {
                ...
                {
                    foreach (var framed in frameToSourceFrameSize(buf))
                    {
