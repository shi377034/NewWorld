...
public class vAudioSurface : ScriptableObject
{
    ...
    public void PlayRandomClip(FootStepObject footStepObject)
    {
        ...
        if (audioSource != null)
            audioObject = audioSource.gameObject.Instantiate(footStepObject.sender.position,Quaternion.identity,0) as GameObject;
        else
        {
        ...
        if (particleObject)
        {
            var particle = particleObject.Instantiate(footStepObject.sender.position, footStepObject.sender.rotation,0) as GameObject;
