...
namespace Invector.ItemManager
{
    public class vItemManager : MonoBehaviour
    {
        ...
        public void DropItem(vItem item, int amount)
        {
            ...
            {
                var dropObject = item.dropObject.Instantiate(transform.position, transform.rotation,0) as GameObject;
        ...
        public void DropAllItens(GameObject target = null)
        {
            ...
            for (int i = 0; i < items.Count; i++)
            {
                if (itemReferences.Find(_item => _item.id == items[i].id) == null)
                {
                    ...
                    {
                        var dropObject = items[i].dropObject.Instantiate(transform.position, transform.rotation,0) as GameObject;
                        vItemCollection collection = dropObject.GetComponent<vItemCollection>();
                        if (collection != null)
                        {
                            collection.items.Clear();
            ...
            if (!inEquip)
            {
                ...
                if (equipPoint != null)
                {
                    if (item.originalObject)
                    {
                        ...
                        var equipedObject = item.originalObject.Instantiate(equipTransform.position, equipTransform.rotation,0) as GameObject;
