...
public class vControlAimCanvas : MonoBehaviour
{
    ...
    /// <param name="validPoint">Set if Aim is Valid</param>
    public void SetWordPosition(vThirdPersonController controller,Vector3 wordPosition, bool validPoint = true)
    {
        if (currentAimCanvas == null) return;
        if (validPoint != isValid)
        {
        ...
        if (controller.input.magnitude > movementSensibility || tpCamera.movementSpeed.magnitude > movementSensibility)
        {
